...
const char *__fastcall GetScriptStringParam(CRunningScript *thread, int dummy, char *buff, int buffLen)
{
    ...
    if (IsImmInteger(paramType) || isVariableInt)
    {
        ...
        {
            LOG_WARNING(thread, "Invalid '0x%X' pointer of input string argument %s in script %s", str,
    ...
    LOG_WARNING(thread, "Argument %s expected to be string, got %s in script %s", GetParamInfo().c_str(),
...
void CScriptEngine::Inject(CCodeInjector &inj)
{
    ...
    inj.ReplaceFunction(HOOK_DrawScriptText, gvm.TranslateMemoryAddress(MA_CALL_DRAW_SCRIPT_TEXTS_AFTER_FADE),
                        &DrawScriptTextAfterFade_Orig);
    inj.ReplaceFunction(HOOK_DrawScriptText, gvm.TranslateMemoryAddress(MA_CALL_DRAW_SCRIPT_TEXTS_BEFORE_FADE),
...
void CScriptEngine::LoadState(int saveSlot)
{
    ...
    try
    {
        ...
        if (ss.is_open())
        {
            ...
            for (size_t i = 0; i < safe_header.n_stopped_threads; ++i)
                InactiveScriptHashes.insert(stopped_info[i]);
            TRACE("Finished. Loaded %u cleo variables, %u saved threads info, %u stopped threads info", 0x400,
...
void CScriptEngine::SaveState()
{
    try
    {
        ...
        if (ss.is_open())
        {
            ...
            TRACE("Done. Saved %u cleo variables, %u saved threads, %u stopped threads", 0x400, header.n_saved_threads,
                  header.n_stopped_threads);
        }
        else
        {
