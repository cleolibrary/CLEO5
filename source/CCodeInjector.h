...
namespace CLEO
{
    ...
    union memory_pointer {
        ...
        template <typename T> inline operator T*() { return reinterpret_cast<T*>(pointer); }
        template <typename T> inline memory_pointer& operator=(T* p) { return *this = reinterpret_cast<void*>(p); }
    };
    ...
    class CCodeInjector
    {
      ...
      public:
        ...
        template <typename T> void ReplaceFunction(T* funcPtr, memory_pointer position, T** origFuncPtr = nullptr)
        {
            ...
            {
                TRACE("Replaced call at: 0x%08X, original function was: 0x%08X", (DWORD)position, (DWORD)*origFuncPtr);
            }
        }
