...
namespace CLEO
{
    ...
    SCRIPT_VAR* GetScriptVar(CLEO::CRunningScript* script, bool global, size_t index)
    {
        SCRIPT_VAR* vars;
        if (global)
    ...
    struct ScriptParamInfo
    {
        ...
        ScriptParamInfo(CLEO::CRunningScript* script)
        {
            ...
            {
                // immediate numbers
                case DT_BYTE: value.dwParam = script->ReadByte(); break;
                case DT_WORD: value.dwParam = script->ReadWord(); break;
                case DT_DWORD: value.dwParam = script->ReadDword(); break;
                case DT_FLOAT: value.dwParam = script->ReadDword(); break; // 4 bytes
        ...
        eDataType GetBaseDataType() const
        {
            switch (type)
            {
                ...
                case DT_VAR_ARRAY:
                case DT_LVAR_ARRAY:
                    if (arrayType == AT_INT) return DT_DWORD; // int
                    else if (arrayType == AT_FLOAT) return DT_FLOAT; // float
                    else return DT_VAR; // unknown
