    ...
    if (result == OR_NONE) // opcode not proccessed yet
    {
        ...
        if (result == OR_ERROR)
        {
            ...
            SHOW_ERROR("Opcode [%04X] not found!%s\nCalled in script %s\nScript suspended.", opcode,
                       extensionMsg.c_str(), ScriptInfoStr(thread).c_str());
...
int ReadFormattedString(CRunningScript* thread, char* outputStr, DWORD len, const char* format)
{
    ...
    if (len > 1 && format != nullptr)
    {
        while (*iter)
        {
            ...
            if (*iter == '%')
            {
                ...
                if (iter[1] == '\0')
                {
                    LOG_WARNING(thread, "ReadFormattedString encountered incomplete format specifier in script %s",
                                ScriptInfoStr(thread).c_str());
                    SkipUnusedVarArgs(thread);
                    return -1; // error
                }
    ...
    _ReadFormattedString_OutOfMemory: // jump here on error
        ...
        LOG_WARNING(thread, "Target buffer too small (%d) to read whole formatted string in script %s", len,
                    ScriptInfoStr(thread).c_str());
        SkipUnusedVarArgs(thread);
        outputStr[len - 1] = '\0';
        return -1; // error
                                                    ...
                                                    DWORD returnArgCount, bool strictArgCount)
{
    ...
    if (returnArgs)
    {
        ...
        if (returnArgCount > nVarArg)
        {
            SHOW_ERROR("Opcode [%04X] declared %d args, but %d was provided in script %s\nScript suspended.", opcode,
                       returnArgCount, nVarArg, ScriptInfoStr(thread).c_str());
            return thread->Suspend();
        }
    ...
    if (returnArgs)
    {
        ...
        if (returnSlotCount != returnArgCount)
        {
            if (strictArgCount)
            {
                SHOW_ERROR_COMPAT("Opcode [%04X] returned %d params, while function caller expected %d in script "
                                  "%s\nScript suspended.",
            ...
            {
                LOG_WARNING(thread, "Opcode [%04X] returned %d params, while function caller expected %d in script %s",
                            opcode, returnArgCount, returnSlotCount, cs->GetInfoStr().c_str());
            }
        }
...
OpcodeResult __stdcall CCustomOpcodeSystem::opcode_0A94(CRunningScript* thread)
{
    ...
    else
    {
        if (cs)
            delete cs;
        SkipUnusedVarArgs(thread);
        LOG_WARNING(0, "[0A94] Failed to load mission '%s' from script '%s'.", filename.c_str(),
                    thread->GetName().c_str());
    }
...
OpcodeResult __stdcall CCustomOpcodeSystem::opcode_0AB1(CRunningScript* thread)
{
    ...
    if (!moduleTxt.empty())
    {
        ...
        if (!scriptRef.Valid())
        {
            SHOW_ERROR("Not found module '%s' export '%s', requested by opcode [0AB1] in script %s", modulePath.c_str(),
                       moduleTxt.c_str(), ScriptInfoStr(thread).c_str());
            return thread->Suspend();
        }
    ...
    if (nParams)
    {
        ...
        {
            SHOW_ERROR("Opcode [0AB1] declared %d input args, but provided %d in script %s\nScript suspended.", nParams,
                       nVarArg, ScriptInfoStr(thread).c_str());
            return thread->Suspend();
        }

        if (nParams > 32)
        {
            SHOW_ERROR("Argument count %d is out of supported range (32) of opcode [0AB1] in script %s", nParams,
                       ScriptInfoStr(thread).c_str());
            return thread->Suspend();
        }
    }
...
OpcodeResult __stdcall CCustomOpcodeSystem::opcode_0AB2(CRunningScript* thread)
{
    ...
    if (returnParamCount)
    {
        ...
        {
            LOG_WARNING(thread, "Opcode [0AB2] declared %d return args, but provided %d in script %s",
                        declaredParamCount, returnParamCount - 1, ScriptInfoStr(thread).c_str());
        }
