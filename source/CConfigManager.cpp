...
namespace CLEO
{
    ...
    {
        char buffer[64];
        GetPrivateProfileStringA(section, key, "", buffer, sizeof(buffer), CConfigManager::GetConfigPath());

        if (buffer[0] == '\0') return defaultValue;

        char* end;
        // Try to parse as hex if it starts with "0x" or "0X", otherwise parse as decimal
    ...
    bool CConfigManager::WriteInt(const char* section, const char* key, int value)
    {
        ...
        return WritePrivateProfileStringA(section, key, buffer, CConfigManager::GetConfigPath()) != 0;
    }
    ...
    {
        return WritePrivateProfileStringA(section, key, value, CConfigManager::GetConfigPath()) != 0;
    }
