...
CCustomScript::CCustomScript(const char* szFileName, bool bIsMiss, CRunningScript* parent, int label) : CRunningScript()
{
    ...
    try
    {
        ...
        {
            if (!parent)
                throw logic_error("Trying to create external thread from label without parent thread");

            if (!parent->IsCustom())
                throw logic_error("Only custom threads can spawn children threads from label");
        ...
        else
        {
            ...
            if (!FS::is_regular_file(path))
            {
                if (path.extension() == cs_ext)
                {
                    ...
                    else
                    {
                        ...
                        {
                            throw logic_error("File does not exists");
                        }
                    }
                }
                else
            ...
            ifstream is(path.string().c_str(), ios::binary);
            ...
            if (bIsMiss)
            {
                if (CTheScripts::bAlreadyRunningAMissionScript)
                    throw logic_error("Starting of custom mission when other mission loaded");
...
void CCustomScript::Process()
{
    ...
    memcpy(m_scriptSprites.data(), CTheScripts::ScriptSprites, sizeof(m_scriptSprites)); // store loaded textures
...
void CCustomScript::Draw(uchar beforeFade)
{
    if (m_useTextCommands == false ||
        (m_scriptDraws.empty() && m_scriptTexts.empty()))
    {
    ...
    memcpy(CTheScripts::IntroRectangles, m_scriptDraws.data(), m_scriptDraws.size() * sizeof(decltype(m_scriptDraws)::value_type));

    memcpy(CTheScripts::ScriptSprites, m_scriptSprites.data(), sizeof(m_scriptSprites));

    CTheScripts::NumberOfIntroTextLinesThisFrame = (WORD)m_scriptTexts.size();
    memcpy(CTheScripts::IntroTextLines, m_scriptTexts.data(), m_scriptTexts.size() * sizeof(decltype(m_scriptTexts)::value_type));

    // draw
    if (beforeFade) CleoInstance.ScriptEngine.DrawScriptTextBeforeFade_Orig(beforeFade);
...
string CCustomScript::GetScriptFileFullPath() const
{
    string path = GetScriptFileDir();
    path += '\\';
...
string CCustomScript::ResolvePath(const char* path, const char* customWorkDir) const
{
    if (path == nullptr)
    {
        return {};
...
string CCustomScript::GetInfoStr(bool currLineInfo) const
{
    ostringstream ss;
