...
#define VALIDATE_STREAM()                                                                                              \
    ...
    {                                                                                                                  \
        SHOW_ERROR("Invalid or already closed '0x%X' audio stream handle param in script %s \nScript suspended.",      \
...
class Audio
{
  public:
    ...
    static OpcodeResult __stdcall opcode_0AAD(CLEO::CRunningScript* thread)
    {
        ...
        if (stream)
        {
            ...
            case eStreamAction::Resume:
                ...
                break;
            default:
                LOG_WARNING(thread, "Unknown AudioStreamAction (%d) in script %s", action,
    ...
    static OpcodeResult __stdcall opcode_0AC2(CLEO::CRunningScript* thread)
    {
        ...
        {
            LOG_WARNING(thread, "3d audio stream command used with non-3d audio stream in script %s",
                        ScriptInfoStr(thread).c_str());
        }
