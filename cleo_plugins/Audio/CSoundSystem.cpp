...
bool CSoundSystem::Init()
{
    ...
    if (forceIndex != -1)
    {
        ...
        {
            LOG_WARNING(0, "Failed to force select device #%d! Selecting default audio device #%d: %s", forceIndex,
        ...
        BASS_Set3DFactors(1.0f, 0.0f, 1.0f))
    {
        ...
        DWORD floatable = BASS_StreamCreate(44100, 1, BASS_SAMPLE_FLOAT, NULL,
...
void CSoundSystem::Process()
{
    ...
    else // not in menu
    {
        ...
        if (!skipFrame)
        {
            ...
            BASS_Set3DPosition(&toBass(position), &toBass(velocity), &toBass(direction), &toBass(TheCamera.GetUp()));
        }
