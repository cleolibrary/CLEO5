...
CLEO::OpcodeResult ScriptLog::OnScriptOpcodeProcessAfter(CLEO::CRunningScript* script, DWORD opcode, CLEO::OpcodeResult result)
{
    ...
    if (m_currCommandReturnParams != nullptr)
    {
        ...
        for (size_t i = command->inputArguments; i < command->arguments.size(); i++)
        {
            if (line.length() > lineEmptyLen) line += ", "; // separator
    ...
    {
        if (line.length() > lineEmptyLen) line += ", ";
        line += "condition: ";
        line += (m_conditionResultValue ^ script->NotFlag) ? "true" : "false";
    }

    if (line.length() > lineEmptyLen) LogLineAppend(line);
