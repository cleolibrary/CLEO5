...
project "CLEO"
    ...
    filter "configurations:Debug"
        postbuildcommands {
            '{COPY} "%{cfg.targetdir}/CLEO.lib" "%{wks.location}/../cleo_sdk/"',
            'if defined GTA_SA_DIR ( taskkill /IM gta_sa.exe /F /FI "STATUS eq RUNNING" & xcopy /Y "%{cfg.targetdir}/CLEO.asi" "%GTA_SA_DIR%\\" & xcopy /Y "%{cfg.targetdir}/CLEO.pdb" "%GTA_SA_DIR%\\" )'
        }
...
local function define_cleo_plugin(config)
    ...
    project(name)
        ...
        filter "configurations:Debug"
            postbuildcommands {
                'if defined GTA_SA_DIR ( taskkill /IM gta_sa.exe /F /FI "STATUS eq RUNNING" & xcopy /Y "%{cfg.targetdir}/' .. target_name .. '.cleo" "%GTA_SA_DIR%\\cleo\\cleo_plugins\\" & xcopy /Y "%{cfg.targetdir}/' .. target_name .. '.pdb" "%GTA_SA_DIR%\\cleo\\cleo_plugins\\" )'
            }
...
define_cleo_plugin {
    name = "Audio",
    extra_includedirs = { "cleo_plugins/Audio/bass" },
