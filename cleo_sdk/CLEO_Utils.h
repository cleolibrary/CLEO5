...
critical errors. Please mind those expand into multiple lines, so CAN NOT be used in places where single code line is
expected! (like 'if' condition body without brackets)
...
null-terminated string OPCODE_READ_PARAM_STRING_LEN(varName, maxLength) // same as above, but text length is clamped to
maxLength OPCODE_READ_PARAM_STRING_FORMATTED(varName) // reads "format" string argument, then all var-args. Creates
variable named 'varName' containing formatted text. Creates 'varNameOk' variable with pointer to the text, or nullptr if
user provided invalid arguments OPCODE_READ_PARAMS_FORMATTED(format, varName) // reads all var-args and tries to put
them into formatted string. Creates variable named 'varName' containing formatted text. Creates 'varNameOk' variable
with pointer to the text, or nullptr if user provided invalid arguments OPCODE_READ_PARAM_FILEPATH(varName) // reads
param and creates const char* variable named 'varName' with pointer to resolved, null-terminated, filepath
...
#define OPCODE_VALIDATE_POINTER(x)                                                                                     \
    ...
    {                                                                                                                  \
        SHOW_ERROR("Invalid '0x%X' pointer argument in script %s \nScript suspended.", x,                              \
...
static const char* _readParamText(CRunningScript* thread, char* buffer, size_t bufferSize)
{
    ...
    if (!_paramWasString())
    {
        SHOW_ERROR("Input argument %s expected to be string, got %s in script %s\nScript suspended.",
                   GetParamInfo(1).c_str(), ToKindStr(_lastParamType, _lastParamArrayType),
                   ScriptInfoStr(thread).c_str());
        thread->Suspend();
    ...
    auto str = CLEO_ReadStringPointerOpcodeParam(thread, buffer,
    ...
    if (str == nullptr) // reading string failed
    {
        ...
        {
            SHOW_ERROR("Invalid '0x%X' pointer of input string argument %s in script %s",
...
// macros for reading opcode input params. Performs type validation, throws error and suspends script if user provided
// invalid argument type TOD: add range checks for limited size types?

#define OPCODE_READ_PARAM_BOOL()                                                                                       \
    _readParam(thread).dwParam != false;                                                                               \
    if (!_paramWasInt())                                                                                               \
    {                                                                                                                  \
        SHOW_ERROR("Input argument %s expected to be integer, got %s in script %s\nScript suspended.",                 \
...
#define OPCODE_READ_PARAM_FLOAT()                                                                                      \
    ...
    {                                                                                                                  \
        SHOW_ERROR_COMPAT("Input argument %s expected to be float, got %s in script %s\nScript suspended.",            \
                          GetParamInfo().c_str(), CLEO::ToKindStr(_lastParamType, _lastParamArrayType),                \
                          CLEO::ScriptInfoStr(thread).c_str());                                                        \
        return thread->Suspend();                                                                                      \
    }
...
#define OPCODE_READ_PARAM_ANY32()                                                                                      \
    ...
    {                                                                                                                  \
        SHOW_ERROR("Input argument %s expected to be int or float, got %s in script %s\nScript suspended.",            \
                   GetParamInfo().c_str(), CLEO::ToKindStr(_lastParamType, _lastParamArrayType),                       \
                   CLEO::ScriptInfoStr(thread).c_str());                                                               \
        return thread->Suspend();                                                                                      \
    }
...
#define OPCODE_READ_PARAM_FILEPATH(_varName)                                                                           \
    ...
    {                                                                                                                  \
        SHOW_ERROR("Forbidden file path '%s' outside game directories in script %s \nScript suspended.", ##_varName,   \
                   ScriptInfoStr(thread).c_str());                                                                     \
        return thread->Suspend();                                                                                      \
    }
...
#define OPCODE_READ_PARAM_PED_HANDLE()                                                                                 \
    ...
    {                                                                                                                  \
        SHOW_ERROR("Invalid character handle '0x%X' input argument %s in script %s \nScript suspended.",               \
                   _paramsArray[0].dwParam, GetParamInfo().c_str(), ScriptInfoStr(thread).c_str());                    \
        return thread->Suspend();                                                                                      \
    }
...
#define OPCODE_READ_PARAM_OUTPUT_VAR_ANY32()                                                                           \
    ...
    {                                                                                                                  \
        SHOW_ERROR("Output argument %s expected to be variable int or float, got %s in script %s\nScript suspended.",  \
                   GetParamInfo().c_str(), CLEO::ToKindStr(_lastParamType, _lastParamArrayType),                       \
                   CLEO::ScriptInfoStr(thread).c_str());                                                               \
        return thread->Suspend();                                                                                      \
    }
...
#define OPCODE_READ_PARAM_OUTPUT_VAR_FLOAT()                                                                           \
    ...
    {                                                                                                                  \
        SHOW_ERROR_COMPAT("Output argument %s expected to be variable float, got %s in script %s\nScript suspended.",  \
                          GetParamInfo().c_str(), CLEO::ToKindStr(_lastParamType, _lastParamArrayType),                \
                          CLEO::ScriptInfoStr(thread).c_str());                                                        \
        return thread->Suspend();                                                                                      \
    }
...
// macros for writing opcode output params. Performs type validation, throws error and suspends script if user provided
...
#define OPCODE_WRITE_PARAM_ANY32(_value)                                                                               \
    ...
    {                                                                                                                  \
        SHOW_ERROR("Output argument %s expected to be int or float variable, got %s in script %s\nScript suspended.",  \
                   GetParamInfo().c_str(), CLEO::ToKindStr(_lastParamType, _lastParamArrayType),                       \
                   CLEO::ScriptInfoStr(thread).c_str());                                                               \
        return thread->Suspend();                                                                                      \
    }
