...
LPCSTR WINAPI CLEO_ReadStringOpcodeParam(CRunningScript* thread, char* buff = nullptr, int buffSize = 0); // writes null terminated string into buffer. If no buffer provided then internal one is used. Returns pointer to the result buffer or nullptr on fail
LPCSTR WINAPI CLEO_ReadStringPointerOpcodeParam(CRunningScript* thread, char* buff = nullptr, int buffSize = 0); // writes null terminated string into buffer. Whenever possible returns pointer to the original, null terminated, source string (which ma be longer than provided buffer). Returns nullptr on fail.
